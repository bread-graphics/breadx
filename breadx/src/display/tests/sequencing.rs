...
use crate::{
    connection::with_test_connection,
    display::{from_reply_fds_request, from_reply_request, from_void_request, RawRequest},
    Fd,
};
use alloc::vec::Vec;
use x11rb_protocol::{
    protocol::xproto::SetupRequest,
    x11_utils::{ReplyFDsRequest, ReplyRequest, Serialize, TryParseFd, VoidRequest},
};
...
impl Action {
    ...
    pub(crate) fn void_request(
        ...
        from_void_request(void, discard_reply, move |mut req| {
            req.format(ext_opcode, self.dummy_max_len)
    ...
    pub(crate) fn setup_request(&mut self, sr: &SetupRequest) -> &mut Self {
    ...
    pub(crate) fn respond_with_setup(&mut self) -> &mut Self {
