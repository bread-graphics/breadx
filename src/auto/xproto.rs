...
impl AsByteSequence for SetupRequest {
    ...
    fn size(&self) -> usize {
        self.byte_order.size()
            ...
            + {
                let block_len: usize = self.authorization_protocol_data.len();
                let pad: usize = buffer_pad(block_len, 4);
                block_len + pad
            }
    }
...
impl AsByteSequence for DestroyWindowRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.window.size()
    }
}
...
impl AsByteSequence for ChangeSaveSetRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + self.mode.size() + self.window.size()
    }
}
...
impl AsByteSequence for CirculateWindowRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + self.direction.size() + self.window.size()
    }
}
...
impl AsByteSequence for UngrabPointerRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.time.size()
    }
}
...
impl AsByteSequence for AllowEventsRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + self.mode.size() + self.time.size()
    }
}
...
impl AsByteSequence for GetInputFocusRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + self.revert_to.size() + self.focus.size()
    }
}
...
impl AsByteSequence for QueryKeymapRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.keys.size()
    }
}
...
impl AsByteSequence for CloseFontRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.font.size()
    }
}
...
impl AsByteSequence for QueryTextExtentsRequest {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.length.as_bytes(&mut bytes[index..]);
        index += self.font.as_bytes(&mut bytes[index..]);
        index += self.string.as_bytes(&mut bytes[index..]);
        index += self.draw_direction.as_bytes(&mut bytes[index..]);
        index += self.font_ascent.as_bytes(&mut bytes[index..]);
        index += self.font_descent.as_bytes(&mut bytes[index..]);
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (font, sz): (Fontable, usize) = <Fontable>::from_bytes(&bytes[index..])?;
        index += sz;
        let (string, sz): ([Char2b; 0], usize) = <[Char2b; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        let (draw_direction, sz): (FontDraw, usize) = <FontDraw>::from_bytes(&bytes[index..])?;
        index += sz;
        let (font_ascent, sz): (Int16, usize) = <Int16>::from_bytes(&bytes[index..])?;
...
impl AsByteSequence for SetFontPathRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + ::core::mem::size_of::<Card16>() + 2 + {
...
impl AsByteSequence for FreePixmapRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.pixmap.size()
    }
}
...
impl AsByteSequence for SetClipRectanglesRequest {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.clip_x_origin.as_bytes(&mut bytes[index..]);
        index += self.clip_y_origin.as_bytes(&mut bytes[index..]);
        index += self.rectangles.as_bytes(&mut bytes[index..]);
        index
    }
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        index += sz;
        let (rectangles, sz): ([Rectangle; 0], usize) =
            <[Rectangle; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
...
impl AsByteSequence for FreeGcRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.gc.size()
    }
}
...
impl AsByteSequence for PolyPointRequest {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.drawable.as_bytes(&mut bytes[index..]);
        index += self.gc.as_bytes(&mut bytes[index..]);
        index += self.points.as_bytes(&mut bytes[index..]);
        index
    }
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (gc, sz): (Gcontext, usize) = <Gcontext>::from_bytes(&bytes[index..])?;
        index += sz;
        let (points, sz): ([Point; 0], usize) = <[Point; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            PolyPointRequest {
                req_type: req_type,
...
impl AsByteSequence for PolySegmentRequest {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.drawable.as_bytes(&mut bytes[index..]);
        index += self.gc.as_bytes(&mut bytes[index..]);
        index += self.segments.as_bytes(&mut bytes[index..]);
        index
    }
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (gc, sz): (Gcontext, usize) = <Gcontext>::from_bytes(&bytes[index..])?;
        index += sz;
        let (segments, sz): ([Segment; 0], usize) = <[Segment; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            PolySegmentRequest {
                req_type: req_type,
...
impl AsByteSequence for PolyArcRequest {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.drawable.as_bytes(&mut bytes[index..]);
        index += self.gc.as_bytes(&mut bytes[index..]);
        index += self.arcs.as_bytes(&mut bytes[index..]);
        index
    }
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (gc, sz): (Gcontext, usize) = <Gcontext>::from_bytes(&bytes[index..])?;
        index += sz;
        let (arcs, sz): ([Arc; 0], usize) = <[Arc; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            PolyArcRequest {
                req_type: req_type,
...
impl AsByteSequence for PutImageRequest {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        index += sz;
        index += 2;
        let (data, sz): ([Byte; 0], usize) = <[Byte; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            PutImageRequest {
                req_type: req_type,
...
impl AsByteSequence for PolyText8Request {
    ...
    fn as_bytes(&self, bytes: &mut [u8]) -> usize {
        ...
        index += self.x.as_bytes(&mut bytes[index..]);
        index += self.y.as_bytes(&mut bytes[index..]);
        index += self.items.as_bytes(&mut bytes[index..]);
        index
    }
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (y, sz): (Int16, usize) = <Int16>::from_bytes(&bytes[index..])?;
        index += sz;
        let (items, sz): ([Byte; 0], usize) = <[Byte; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            PolyText8Request {
                req_type: req_type,
...
impl AsByteSequence for FreeColormapRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.cmap.size()
    }
}
...
impl AsByteSequence for CopyColormapAndFreeRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.mid.size() + self.src_cmap.size()
    }
}
...
impl AsByteSequence for FreeColorsRequest {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (plane_mask, sz): (Card32, usize) = <Card32>::from_bytes(&bytes[index..])?;
        index += sz;
        let (pixels, sz): ([Card32; 0], usize) = <[Card32; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            FreeColorsRequest {
                req_type: req_type,
...
impl AsByteSequence for StoreColorsRequest {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        let (cmap, sz): (Colormap, usize) = <Colormap>::from_bytes(&bytes[index..])?;
        index += sz;
        let (items, sz): ([Coloritem; 0], usize) = <[Coloritem; 0]>::from_bytes(&bytes[index..])?;
        index += sz;
        Some((
            StoreColorsRequest {
                req_type: req_type,
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.cmap.size() + self.items.size()
    }
}
...
impl AsByteSequence for FreeCursorRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.cursor.size()
    }
}
...
impl AsByteSequence for ListExtensionsRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + ::core::mem::size_of::<Card8>() + self.length.size() + 24 + {
            let block_len: usize = self.names.iter().map(|i| i.size()).sum();
            let pad: usize = buffer_pad(block_len, ::core::mem::align_of::<Str>());
            block_len + pad
        }
...
impl AsByteSequence for GetKeyboardMappingRequest {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        index += sz;
        index += 1;
        let (len0, sz): (u16, usize) = <u16>::from_bytes(&bytes[index..])?;
        index += sz;
        index += 1;
        let (first_keycode, sz): (Keycode, usize) = <Keycode>::from_bytes(&bytes[index..])?;
        index += sz;
...
impl AsByteSequence for BellRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + self.percent.size()
    }
}
...
impl AsByteSequence for KillClientRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + 1 + self.length.size() + 1 + self.resource.size()
    }
}
...
impl AsByteSequence for GetModifierMappingRequest {
    ...
    #[inline]
    fn size(&self) -> usize {
        self.req_type.size() + self.keycodes_per_modifier.size() + self.length.size() + 24 + {
            let block_len: usize = self.keycodes.iter().map(|i| i.size()).sum();
            let pad: usize = buffer_pad(block_len, ::core::mem::align_of::<Keycode>());
            block_len + pad
        }
...
impl AsByteSequence for KeymapNotifyEvent {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        index += sz;
        Some((KeymapNotifyEvent { keys: keys }, index))
    }
    #[inline]
    fn size(&self) -> usize {
        4 + self.keys.size()
