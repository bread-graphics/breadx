// This file was automatically generated by the flutterbug_generator crate from "/home/jtnunley/Programming/Projects/flutterbug/xml/xc_misc.xml".
...
impl AsByteSequence for GetXIDListReply {
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, usize)> {
        ...
        index += sz;
        index += 1;
        let (len2018179485, sz): (Card32, usize) = <Card32>::from_bytes(&bytes[index..])?;
        index += sz;
        index += 20;
        let (ids, sz): (Vec<Card32>, usize) = vector_from_bytes(bytes, len2018179485 as usize)?;
        index += sz;
        Some((
            Self {
                sequence,
