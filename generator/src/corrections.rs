...
pub fn corrections(body: String, fname: &OsStr) -> String {
  let re1 = Regex::new(r"Vec < Char > = unsafe \{ clone_from_ptr").unwrap();
  let re2 = Regex::new(r"\bChar\b").unwrap();
  let re3 = Regex::new(r"\bBool\b").unwrap();
  let re4 = Regex::new(r"\bStr\b").unwrap();
  let re5 = Regex::new(r"\bVoid\b").unwrap();

  let body = re4.replace_all(&body, "String");
  let body = re1.replace_all(&body, "String = unsafe { string_from_ptr");
  let body = re2.replace_all(&body, "bool");
  let body = re3.replace_all(&body, "::std::os::raw::c_char");
  let body = re5.replace_all(&body, "()");
